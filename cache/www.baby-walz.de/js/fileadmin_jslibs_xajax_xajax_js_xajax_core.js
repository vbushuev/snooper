        ...
        return {
            onWaiting: function() {
                if (xajax.config.baseDocument.body)
                    xajax.config.baseDocument.body.style.cursor =
                    'wait';
            },
            onComplete: function() {
                xajax.config.baseDocument.body.style.cursor =
...
xajax.tools.getFormValues = function(parent) {
    ...
    if (parent)
        if (parent.childNodes)
            xajax.tools._getFormValues(aFormValues, parent.childNodes,
                submitDisabledElements, prefix);
    return aFormValues;
}
xajax.tools._getFormValues = function(aFormValues, children,
    submitDisabledElements, prefix) {
    var iLen = children.length;
    for (var i = 0; i < iLen; ++i) {
        var child = children[i];
        if ('undefined' != typeof child.childNodes)
            xajax.tools._getFormValues(aFormValues, child.childNodes,
                submitDisabledElements, prefix);
        xajax.tools._getFormValue(aFormValues, child,
            submitDisabledElements, prefix);
    }
}
xajax.tools._getFormValue = function(aFormValues, child, submitDisabledElements,
    prefix) {
    if (!child.name)
        return;
    if (child.disabled)
...
xajax.responseProcessor.xml = function(oRequest) {
    ...
    } else if (xt.arrayContainsValue(xx.responseRedirectCodes, oRequest.request
            .status)) {
        xcb.execute([gcb, lcb], 'onRedirect', oRequest);
        window.location = oRequest.request.getResponseHeader("location");
        xx.completeResponse(oRequest);
    } else if (xt.arrayContainsValue(xx.responseErrorsForAlert, oRequest.request
            .status)) {
...
if ('undefined' == typeof window.addEventListener) {
    xajax.forms.getInput = function(type, name, id) {
        return xajax.config.baseDocument.createElement('<input type="' +
...
if (window.addEventListener) {
    xajax.events.addHandler = function(element, event, fun) {
        ...
        eval('element.addEventListener("' + event + '", ' + fun +
            ', false);');
        return true;
    }
} else {
...
if (window.addEventListener) {
    xajax.events.removeHandler = function(element, event, fun) {
        if ('string' == typeof element)
            element = xajax.$(element);
        event = xajax.tools.stripOnPrefix(event);
        eval('element.removeEventListener("' + event + '", ' + fun +
            ', false);');
        return true;
    }
} else {
...
xajax.responseErrorsForAlert = ['400', '401', '402', '403', '404', '500', '501',
    '502', '503'
];
...
xajax.commands["iia"] = function(args) {
    ...
    return xajax.forms.insertInputAfter(args.id, args.type, args.data, args
        .property);
}
...
xajax.initializeRequest = function(oRequest) {
    ...
    oRequest.status = (oRequest.statusMessages) ? xc.status.update() : xc.status
        .dontUpdate();
    oRequest.cursor = (oRequest.waitCursor) ? xc.cursor.update() : xc.cursor
        .dontUpdate();
...
xajax.prepareRequest = function(oRequest) {
    ...
    if ('undefined' != typeof oRequest.userName && 'undefined' != typeof oRequest
        .password) {
...
xajax.parseChildren = function(child, obj) {
    ...
    if (0 < child.childNodes.length) {
        if (1 < child.childNodes.length) {
            var grandChild = child.firstChild;
            do {
                if ('#cdata-section' == grandChild.nodeName || '#text' ==
                    grandChild.nodeName) {
            ...
            } else if ('#cdata-section' == grandChild.nodeName || '#text' ==
                grandChild.nodeName) {
                obj.data = grandChild.data;
            }
        }
