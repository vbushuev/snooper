var Prototype = {
    ...
    Browser: (function() {
        ...
        return {
            ...
            Gecko: b.indexOf("Gecko") > -1 && b.indexOf("KHTML") ===
                -1,
...
var Class = (function() {
    ...
    function c(l) {
        ...
        for (var e = 0, h = f.length; e < h; e++) {
            var k = f[e],
                j = l[k];
            if (g && Object.isFunction(j) && j.argumentNames()[0] ==
                "$super") {
                var m = j;
                j = (function(i) {
                    return function() {
...
(function() {
    var C = Object.prototype.toString,
        ...
        h = "[object Array]",
        x = "[object Date]",
        i = window.JSON && typeof JSON.stringify === "function" && JSON.stringify(
            0) === "0" && typeof JSON.stringify(Prototype.K) ===
        "undefined";
    ...
    var b = (typeof Array.isArray == "function") && Array.isArray([]) && !
        Array.isArray({});
    if (b) {
        m = Array.isArray
    }
...
(function(c) {
    function b() {
        return this.getUTCFullYear() + "-" + (this.getUTCMonth() + 1).toPaddedString(
                2) + "-" + this.getUTCDate().toPaddedString(2) + "T" + this
            .getUTCHours().toPaddedString(2) + ":" + this.getUTCMinutes().toPaddedString(
                2) + ":" + this.getUTCSeconds().toPaddedString(2) + "Z"
    }
...
var PeriodicalExecuter = Class.create({
    ...
    registerCallback: function() {
        this.timer = setInterval(this.onTimerEvent.bind(this), this
            .frequency * 1000)
    },
    execute: function() {
        this.callback(this)
...
Object.extend(String.prototype, (function() {
    var NATIVE_JSON_PARSE_SUPPORT = window.JSON && typeof JSON.parse ===
        "function" && JSON.parse('{"test": true}').test;
    ...
    function truncate(length, truncation) {
        ...
        return this.length > length ? this.slice(0, length -
            truncation.length) + truncation : String(this)
    }
    ...
    function underscore() {
        return this.replace(/::/g, "/").replace(
            ...
            /([a-z\d])([A-Z])/g, "$1_$2").replace(/-/g, "_").toLowerCase()
    }
    ...
    function inspect(useDoubleQuotes) {
        var escapedString = this.replace(/[\x00-\x1f\\]/g, function(
            character) {
            if (character in String.specialChar) {
                return String.specialChar[character]
            }
            return "\\u00" + character.charCodeAt().toPaddedString(
                2, 16)
        });
        if (useDoubleQuotes) {
...
var Template = Class.create({
    ...
    evaluate: function(a) {
        ...
        return this.template.gsub(this.pattern, function(d) {
            ...
            while (d != null) {
                var c = d[1].startsWith("[") ? d[2].replace(
                    /\\\\]/g, "]") : d[1];
                b = b[c];
                if (null == b || "" == d[3]) {
                    break
                }
                g = g.substring("[" == d[3] ? d[1].length :
                    d[0].length);
                d = e.exec(g)
            }
...
var Hash = Class.create(Enumerable, (function() {
    function e(p) {
        this._object = Object.isHash(p) ? p.toObject() : Object.clone(
            p)
    }
...
Ajax.Request = Class.create(Ajax.Base, {
    ...
    request: function(b) {
        ...
        var d = Object.isString(this.options.parameters) ? this.options
            .parameters : Object.toQueryString(this.options.parameters);
        ...
        try {
            ...
            this.transport.onreadystatechange = this.onStateChange.bind(
                this);
    ...
    setRequestHeaders: function() {
        ...
        if (this.method == "post") {
            e["Content-type"] = this.options.contentType + (this.options
                .encoding ? "; charset=" + this.options.encoding :
                "");
            if (this.transport.overrideMimeType && (navigator.userAgent
                    .match(/Gecko\/(\d{4})/) || [0, 2005])[1] <
                2005) {
                e.Connection = "close"
            }
        }
    ...
    respondToReadyState: function(a) {
        ...
        if (c == "Complete") {
            try {
                this._complete = true;
                (this.options["on" + b.status] || this.options["on" +
                        (this.success() ? "Success" : "Failure")] ||
                    Prototype.emptyFunction)(b, b.headerJSON)
            } catch (d) {
                this.dispatchException(d)
            }
            var f = b.getHeader("Content-type");
            if (this.options.evalJS == "force" || (this.options.evalJS &&
                    this.isSameOrigin() && f && f.match(
        ...
        try {
            (this.options["on" + c] || Prototype.emptyFunction)(b,
                b.headerJSON);
...
Ajax.Response = Class.create({
    ...
    _getResponseJSON: function() {
        ...
        if (!a.evalJSON || (a.evalJSON != "force" && !(this.getHeader(
                "Content-type") || "").include(
                "application/json")) || this.responseText.blank()) {
            return null
        }
        try {
...
if (Prototype.BrowserFeatures.XPath) {
    document._getElementsByXPath = function(f, a) {
        var c = [];
        var e = document.evaluate(f, $(a) || document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE,
            null);
...
if (!Node.ELEMENT_NODE) {
    ...
    var b = (function() {
        try {
            ...
            return e.tagName.toLowerCase() === "input" && e.name ===
                "x"
        } catch (f) {
            return false
        }
    ...
    c.Element = function(g, f) {
        ...
        var h = d(g, f) ? e[g].cloneNode(false) : document.createElement(
            g);
...
Element.Methods = {
    ...
    update: (function() {
        var d = (function() {
            ...
            if (g.options && g.options[0]) {
                h = g.options[0].nodeName.toUpperCase() !==
                    "OPTION"
            }
            g = null;
            return h
        ...
        var f = (function() {
            ...
            try {
                ...
                i = !g.firstChild || g.firstChild && g.firstChild
                    .nodeType !== 3
            } catch (h) {
                i = true
            }
        ...
        function e(l, m) {
            ...
            if (c) {
                if (j in Element._insertionTranslations.tags) {
                    ...
                    Element._getContentFromAnonymousElement(j, m.stripScripts())
                        .each(function(i) {
                            l.appendChild(i)
                        })
                } else {
                    if (a && Object.isString(m) && m.indexOf(
                            "<link") > -1) {
                        while (l.firstChild) {
                            l.removeChild(l.firstChild)
                        }
                        var h = Element._getContentFromAnonymousElement(
                            j, m.stripScripts(), true);
                        h.each(function(i) {
                            l.appendChild(i)
                        })
                    } else {
    ...
    insert: function(c, e) {
        c = $(c);
        if (Object.isString(e) || Object.isNumber(e) || Object.isElement(
                e) || (e && (e.toElement || e.toHTML))) {
            e = {
                bottom: e
            }
        }
    ...
    siblings: function(a) {
        a = $(a);
        return Element.previousSiblings(a).reverse().concat(Element.nextSiblings(
            a))
    },
    match: function(b, a) {
        b = $(b);
    ...
    up: function(b, d, a) {
        ...
        return Object.isNumber(d) ? c[d] : Prototype.Selector.find(c, d,
            a)
    },
    down: function(b, c, a) {
        b = $(b);
        ...
        return Object.isNumber(c) ? Element.descendants(b)[c] : Element
            .select(b, c)[a || 0]
    },
    ...
    readAttribute: function(c, a) {
        ...
        if (Prototype.Browser.IE) {
            ...
            if (a.include(":")) {
                return (!c.attributes || !c.attributes[a]) ? null : c.attributes[
                    a].value
            }
        }
    ...
    hasClassName: function(a, b) {
        ...
        var c = a.className;
        return (c.length > 0 && (c == b || new RegExp("(^|\\s)" + b +
            "(\\s|$)").test(c)))
    },
    ...
    removeClassName: function(a, b) {
        ...
        a.className = a.className.replace(new RegExp("(^|\\s+)" + b +
            "(\\s+|$)"), " ").strip();
        return a
    },
    toggleClassName: function(a, b) {
        ...
        return Element[Element.hasClassName(a, b) ? "removeClassName" :
            "addClassName"](a, b)
    },
    ...
    setStyle: function(b, c) {
        ...
        if (Object.isString(c)) {
            b.style.cssText += ";" + c;
            return c.include("opacity") ? b.setOpacity(c.match(
                /opacity:\s*(\d?\.?\d*)/)[1]) : b
        }
        for (var d in c) {
            if (d == "opacity") {
                ...
                e[(d == "float" || d == "cssFloat") ? (Object.isUndefined(
    ...
    setOpacity: function(a, b) {
        a = $(a);
        a.style.opacity = (b == 1 || b === "") ? "" : (b < 0.00001) ? 0 :
            b;
        return a
    },
    ...
    undoPositioned: function(a) {
        ...
        if (a._madePositioned) {
            ...
            a.style.position = a.style.top = a.style.left = a.style.bottom =
                a.style.right = ""
        }
        return a
    },
...
if (Prototype.Browser.Opera) {
    Element.Methods.getStyle = Element.Methods.getStyle.wrap(function(d, b, c) {
        switch (c) {
            ...
            case "width":
                ...
                var a;
                if (c === "height") {
                    a = ["border-top-width", "padding-top",
                        "padding-bottom", "border-bottom-width"
                    ]
                } else {
                    a = ["border-left-width", "padding-left",
    ...
    if (Prototype.Browser.IE) {
        Element.Methods.getStyle = function(a, b) {
            ...
            if (c == "auto") {
                if ((b == "width" || b == "height") && (a.getStyle(
                        "display") != "none")) {
        ...
        $w(
            ...
            Element._attributeTranslations.write.names[a.toLowerCase()] =
                a;
        ...
        if (Prototype.BrowserFeatures.ElementExtensions) {
            (function() {
                ...
                Element.Methods.down = function(c, d, b) {
                    ...
                    return Object.isNumber(d) ? a(c)[d] : Element.select(
                        c, d)[b || 0]
                }
            })()
        }
...
Element.extend = (function() {
    ...
    if (Prototype.BrowserFeatures.SpecificElementExtensions) {
        if (d) {
            return function(h) {
                if (h && typeof h._extendedByPrototype ==
                    "undefined") {
                    var g = h.tagName;
...
document.viewport = {
    ...
    getScrollOffsets: function() {
        return Element._returnOffset(window.pageXOffset || document.documentElement
            .scrollLeft || document.body.scrollLeft, window.pageYOffset ||
...
(function() {
    ...
    function o(F, G, v) {
        ...
        if (y && A) {
            ...
            var D = G.include("left") || G.include("right") || G.include(
                "width");
            var C = G.include("top") || G.include("bottom") || G.include(
                "height");
    ...
    Element.Layout = Class.create(Hash, {
        ...
        _begin: function() {
            ...
            if (u && (w === u)) {
                ...
                if (B === "absolute" || B === "fixed") {
                    ...
                    var C = y.parentNode,
                        z = $(C).getLayout();
                    x = z.get("width") - this.get("margin-left") -
                        this.get("border-left") - this.get(
                            "padding-left") - this.get(
                            "padding-right") - this.get(
                            "border-right") - this.get(
                            "margin-right")
                }
            }
            y.setStyle({
        ...
        toObject: function() {
            var u = $A(arguments);
            var v = (u.length === 0) ? Element.Layout.PROPERTIES :
                u.join(" ").split(" ");
            var w = {};
            v.each(function(x) {
                if (!Element.Layout.PROPERTIES.include(
                        x)) {
                    return
                }
                var y = this.get(x);
        ...
        toCSS: function() {
            var u = $A(arguments);
            var w = (u.length === 0) ? Element.Layout.PROPERTIES :
                u.join(" ").split(" ");
            var v = {};
            w.each(function(x) {
                if (!Element.Layout.PROPERTIES.include(
    ...
    Element.Offset = Class.create({
        ...
        relativeTo: function(u) {
            return new Element.Offset(this.left - u.left, this.top -
                u.top)
        },
        inspect: function() {
    ...
    function s(y) {
        ...
        do {
            u += v.offsetTop || 0;
            x += v.offsetLeft || 0;
            if (v.offsetParent == w && Element.getStyle(v, "position") ==
                "absolute") {
                break
            }
            ...
            t = function(v) {
                ...
                do {
                    ...
                    if (v.offsetParent == document.body) {
                        if (Element.getStyle(v, "position") ==
                            "absolute") {
                            break
                        }
                    }
...
Prototype.Selector = (function() {
    function a() {
        throw new Error(
            'Method "Prototype.Selector.select" must be defined.')
    }

    function c() {
        throw new Error(
            'Method "Prototype.Selector.match" must be defined.')
    }
...
(function() {
    ...
    var b = function(E, u, B, w) {
        ...
        if (C.length > 1 && k.exec(E)) {
            ...
            if (!w && C.length > 1 && u.nodeType === 9 && !x && f.match
                .ID.test(C[0]) && !f.match.ID.test(C[C.length - 1])) {
            ...
            if (u) {
                var J = w ? {
                    expr: C.pop(),
                    set: a(w)
                } : b.find(C.pop(), C.length === 1 && (C[0] === "~" ||
                        C[0] === "+") && u.parentNode ? u.parentNode :
                    u, x);
    ...
    var f = b.selectors = {
        ...
        relative: {
            "+": function(y, e, x) {
                ...
                for (var u = 0, t = y.length, s; u < t; u++) {
                    if ((s = y[u])) {
                        while ((s = s.previousSibling) && s.nodeType !==
                            1) {}
                        y[u] = w || s && s.nodeName === e ? s ||
                            false : s === e
                    }
                }
                if (w) {
            ...
            ">": function(x, s, y) {
                ...
                if (v && !/\W/.test(s)) {
                    ...
                    for (var t = 0, e = x.length; t < e; t++) {
                        var w = x[t];
                        if (w) {
                            x[t] = v ? w.parentNode : w.parentNode ===
                                s
                        }
                    }
                    if (v) {
        ...
        preFilter: {
            ...
            CHILD: function(e) {
                if (e[1] == "nth") {
                    var s = /(-?)(\d*)n((?:\+|-)?\d*)/.exec(e[2] ==
                        "even" && "2n" || e[2] == "odd" &&
                        "2n+1" || !/\D/.test(e[2]) && "0n+" + e[
                            2] || e[2]);
                    e[2] = (s[1] + (s[2] || 1)) - 0;
                    e[3] = s[3] - 0
                }
            ...
            PSEUDO: function(v, s, t, e, w) {
                if (v[1] === "not") {
                    ...
                    if (f.match.POS.test(v[0]) || f.match.CHILD.test(
                            v[0])) {
                        return true
                    }
                }
        ...
        filters: {
            ...
            button: function(e) {
                return "button" === e.type || e.nodeName.toUpperCase() ===
                    "BUTTON"
            },
            input: function(e) {
        ...
        filter: {
            ...
            ID: function(s, e) {
                return s.nodeType === 1 && s.getAttribute("id") ===
                    e
            },
            TAG: function(s, e) {
                return (e === "*" && s.nodeType === 1) || s.nodeName ===
                    e
            },
            CLASS: function(s, e) {
                return (" " + (s.className || s.getAttribute(
                    "class")) + " ").indexOf(e) > -1
            },
            ATTR: function(w, u) {
                var t = u[1],
                    e = f.attrHandle[t] ? f.attrHandle[t](w) : w[t] !=
                    null ? w[t] : w.getAttribute(t),
                    x = e + "",
                    v = u[2],
                    s = u[4];
                return e == null ? v === "!=" : v === "=" ? x === s :
                    v === "*=" ? x.indexOf(s) >= 0 : v === "~=" ? (
                        " " + x + " ").indexOf(s) >= 0 : !s ? x &&
                    e !== false : v === "!=" ? x != s : v === "^=" ?
                    x.indexOf(s) === 0 : v === "$=" ? x.substr(x.length -
                        s.length) === s : v === "|=" ? x === s || x
                    .substr(0, s.length + 1) === s + "-" : false
            },
    ...
    if (document.documentElement.compareDocumentPosition) {
        c = function(s, e) {
            ...
            var t = s.compareDocumentPosition(e) & 4 ? -1 : s === e ? 0 :
                1;
            if (t === 0) {
                o = true
            }
        ...
        if ("sourceIndex" in document.documentElement) {
            ...
            if (document.createRange) {
                c = function(u, s) {
                    ...
                    e.setStart(s, 0);
                    e.setEnd(s, 0);
                    var v = t.compareBoundaryPoints(Range.START_TO_END,
                        e);
                    if (v === 0) {
                        o = true
                    }
        ...
        if (!!document.getElementById(t)) {
            f.find.ID = function(v, w, x) {
                if (typeof w.getElementById !== "undefined" && !x) {
                    ...
                    return u ? u.id === v[1] || typeof u.getAttributeNode !==
                        "undefined" && u.getAttributeNode("id").nodeValue ===
                        v[1] ? [u] : undefined : []
                }
            };
            f.filter.ID = function(w, u) {
                var v = typeof w.getAttributeNode !== "undefined" &&
    ...
    (function() {
        ...
        if (e.firstChild && typeof e.firstChild.getAttribute !==
            "undefined" && e.firstChild.getAttribute("href") !== "#") {
    ...
    if (document.querySelectorAll) {
        (function() {
            ...
            if (t.querySelectorAll && t.querySelectorAll(".TEST").length ===
                0) {
                return
            }
            b = function(x, w, u, v) {
    ...
    if (document.getElementsByClassName && document.documentElement.getElementsByClassName) {
        (function() {
            ...
            f.find.CLASS = function(s, t, u) {
                if (typeof t.getElementsByClassName !== "undefined" &&
                    !u) {
    ...
    var p = function(e) {
        return e.nodeType === 9 && e.documentElement.nodeName !==
            "HTML" || !!e.ownerDocument && e.ownerDocument.documentElement
            .nodeName !== "HTML"
    };
    var g = function(e, y) {
        var u = [],
...
var Form = {
    ...
    serializeElements: function(h, d) {
        ...
        if (d.hash) {
            ...
            c = "";
            b = function(i, j, k) {
                return i + (i ? "&" : "") + encodeURIComponent(j) +
...
Form.Methods = {
    ...
    findFirstElement: function(b) {
        ...
        var a = c.findAll(function(d) {
            return d.hasAttribute("tabIndex") && d.tabIndex >=
                0
        }).sortBy(function(d) {
            return d.tabIndex
        }).first();
...
Form.Element.Methods = {
    ...
    activate: function(a) {
        ...
        try {
            a.focus();
            if (a.select && (a.tagName.toLowerCase() != "input" || !(
...
Abstract.TimedObserver = Class.create(PeriodicalExecuter, {
    ...
    execute: function() {
        var a = this.getValue();
        if (Object.isString(this.lastValue) && Object.isString(a) ?
            this.lastValue != a : String(this.lastValue) != String(
                a)) {
...
Abstract.EventObserver = Class.create({
    ...
    registerFormCallbacks: function() {
        Form.getElements(this.element).each(this.registerCallback,
            this)
    },
    registerCallback: function(a) {
        if (a.type) {
            switch (a.type.toLowerCase()) {
                case "checkbox":
                case "radio":
                    Event.observe(a, "click", this.onElementEvent.bind(
                        this));
                    break;
                default:
                    Event.observe(a, "change", this.onElementEvent.bind(
                        this));
                    break
            }
        }
...
(function() {
    ...
    function d(G) {
        ...
        var F = G.target,
            ...
            H = G.currentTarget;
        if (H && H.tagName) {
            if (E === "load" || E === "error" || (E === "click" && H.tagName
                    .toLowerCase() === "input" && H.type === "radio")) {
                F = H
            }
        }
    ...
    function c(G) {
        ...
        return G.pageX || (G.clientX + (F.scrollLeft || E.scrollLeft) - (F.clientLeft ||
            0))
    }
    ...
    function b(G) {
        ...
        return G.pageY || (G.clientY + (F.scrollTop || E.scrollTop) - (F.clientTop ||
            0))
    }
    ...
    if (window.addEventListener) {
        C.prototype = window.Event.prototype || document.createEvent(
    ...
    function z(H, G, F, E) {
        ...
        I.eventName = G;
        I.memo = F || {};
        if (document.createEvent && typeof H.dispatchEvent == "function") {
            H.dispatchEvent(I)
        } else if (typeof H.fireEvent == "function"){
...
var Position = {
    ...
    withinIncludingScrolloffsets: function(b, a, d) {
        ...
        this.offset = Element.cumulativeOffset(b);
        return (this.ycomp >= this.offset[1] && this.ycomp < this.offset[
                1] + b.offsetHeight && this.xcomp >= this.offset[0] &&
    ...
    overlap: function(b, a) {
        ...
        if (b == "vertical") {
            return ((this.offset[1] + a.offsetHeight) - this.ycomp) / a
                .offsetHeight
        }
        if (b == "horizontal") {
        ...
        b.getElementsByClassName = Prototype.BrowserFeatures.XPath ?
            function(c, e) {
                ...
                for (var d = 0, k, j; k = c[d]; d++) {
                    if (k.className && (j = " " + k.className + " ") && (j.include(
                            f) || (h && h.all(function(i) {
                            return !i.toString().blank() && j.include(
                                " " + i + " ")
                        })))) {
